...
import (
	...
	"riscvue.com/pkg/entities/assessment"
)
...
func (r *Rules) Validate(model interface{}) error {
	...
	assessmentModel, err := assessment.InterfaceToModel(model)
	if err != nil {
		return err
	}
	return Validation.ValidateStruct(assessmentModel,
		//	Validation.Field(&assessmentModel.ID, Validation.Required, is.UUIDv4),
		Validation.Field(&assessmentModel.Name, Validation.Required, Validation.Length(3, 50)),
		Validation.Field(&assessmentModel.CreatedBy, Validation.Required, Validation.Length(3, 50)),
		Validation.Field(&assessmentModel.SecurityFrameworkId, Validation.Required, Validation.Length(3, 50)),
		Validation.Field(&assessmentModel.CustomerId, Validation.Required, Validation.Length(3, 50)),
		Validation.Field(&assessmentModel.MaturityFrameworkId, Validation.Required, Validation.Length(3, 50)),
	)
}
